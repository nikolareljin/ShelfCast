...
require_shelfcast_web_python_3_10() {
  ...
  if ! command -v python3 >/dev/null 2>&1; then
    if declare -F log_error >/dev/null 2>&1; then
      log_error "ShelfCast web-app requires Python 3.10 or newer. Current version: python3 not found"
      log_error "Use a host or Raspberry Pi OS image that provides Python 3.10+ before continuing."
    else
      echo "ShelfCast web-app requires Python 3.10 or newer. Current version: python3 not found" >&2
